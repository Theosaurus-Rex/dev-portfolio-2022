export const About = () => {
    return (
      <div className="flex flex-col items-center content-center space-y-4">
        <p className="text-medium-purple-600 font-semibold font-inter">I like dinosaurs.</p>
        <h2 className="font-semibold font-raleway text-slate-900 text-5xl">About</h2>
        <p className="font-light font-inter text-slate-500 text-xl">
          Currently a Junior Consultant at Alembic working with Elixir, Phoenix
          LiveView and Typescript. Coder Academy alumni, Artist and dinosaur
          enthusiast.
        </p>
      </div>
    );
  }
